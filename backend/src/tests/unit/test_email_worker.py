...
@pytest.mark.asyncio
@patch("src.infrastructure.queue.email_worker.EmailService")
async def test_handle_email_message_missing_code(mock_email_cls):
    mock_service = MagicMock()
    mock_email_cls.return_value = mock_service
    ...
    mock_service.send_activation_code.assert_not_called()
    mock_service.send_access_code.assert_not_called()
